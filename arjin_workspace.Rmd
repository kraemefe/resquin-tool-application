...
### resp_styles
  ...
  <!--  DRS is mentioned as DARS in the help page but dataframe returns it as DRS. I think this is a typo in the help page to be noted -->

## Getting started

Alongside `resquin` itself, we will use several other packages for setup, data preparation and analysis in this tutorial. To easily install and load packages from the repository of CRAN, the Comprehensive R Archive Network, we can use `pacman` package:
...
Since `resquin` is not currently stored in CRAN, we need to install the package from GitHub. For installation, we can use one of the following commands:
...
After installation, we can import the survey data we want to analyze regarding its response quality. For both `resp_distributions` and `resp_styles` to calculate meaningful indicators, we need to import survey data in a wide format, i.e., with only one row per observation unit (respondent). For this tutorial, the raw data has already been uploaded to the GitHub repository and can be imported directly:
```{r import data}
# Import data from github
raw_data <- read.csv("raw-data/ZA5666_v1-0-0.csv", header=TRUE, sep=";", na.strings="NA")
```

<!-- csv format data is stored in the repository under "raw-data" file -->

##  Inspecting data

Before delving into the analysis of response quality, let's have a first look at the distribution of given responses to both multi-item scales by subsetting the relevant items for each:
<!-- as data is stored in an ordered manner, I used the indices instead of manually specifying each variable name -->

```{r data inspection}
# Subset and inspect responses to political trust scale
start_col_trust_responses <- which(colnames(raw_data) == "bbzc078a")
end_col_trust_responses <- which(colnames(raw_data) == "bbzc087a")
trust_responses <- raw_data[,start_col_trust_responses:end_col_trust_responses]
 
# Print the results
trust_responses

# Subset and inspect responses to NEP scale 
start_col_NEP_responses <- which(colnames(raw_data) == "bczd005a")
end_col_NEP_responses <- which(colnames(raw_data) == "bczd019a")
NEP_responses <- raw_data[,start_col_NEP_responses:end_col_NEP_responses]
...
<!-- Is the approach below -finding unique values to identify what's different from the scale values- okay, or should we simply state something like: "Based on the codebook, the following values represent missing data (-22, -33, -77, -99, -111) and will therefore be recoded as NA"? -->

```{r data preparation}
# Get unique values for all items in the trust responses
combined_unique_values_trust <- unique(unlist(lapply(trust_responses, unique)))

# Print the unique values sorted in an ascending order
sort(combined_unique_values_trust)

# Recode missing values to NA for trust responses
trust <- trust_responses %>%
...
# Get unique values for all items in the trust responses
combined_unique_values_NEP <- unique(unlist(lapply(NEP_responses, unique)))

# Print the unique values sorted in an ascending order
sort(combined_unique_values_NEP)
...
To analyze the response distribution of institutional trust and environmental attitudes across *all respondents*, we need to closely examine the response patterns for each scale while also considering the design of the questionnaire and the wording used.

We can calculate and visualize summary statistics for each indicator in the data frame generated by `resp_distributions`. This will help us understand the typical behavior among respondents, as well as the variability that may indicate unusual response patterns. Let’s begin with the institutional trust scale.
...
ggplot(trust_distribution_long, aes(x = Value, y = ..scaled..)) +
...
The density and box plots provide a quick visualization of central tendency and variability across all indicators. Box plots highlight key summary statistics like the median, quartiles, and potential outliers, while density plots complement this by showing the distribution shape and peaks. Together, they give a fuller picture of how responses are distributed.

## Indicators of response distribution for Institutional Trust 

The `resp_distributions` function provides two measures of **central tendency**: `ii_mean` (average response) and `ii_median` (central response) for each respondent across the 10-item scale (ranging from 1 to 7).
-  The mean of `ii_mean` is 3.755, and the median of `ii_mean` is 3.800. This indicates that the distribution of `ii_mean` across all respondents is nearly normal, with a slight skew towards lower values (indicating moderate distrust). 
-  The `ii_median` has a mean value of 3.645 and a median value of 4.000. These values indicates an almost normal distribution for central values, which are neutral or leans slightly towards distrust.
-  Box plots for both indicators show that half of the respondents have trust scores between 3.0 and 4.5, meaning most people hold moderate or slightly distrustful views.  Outliers exist at 7 (complete trust), indicating extreme trust is rare, while extreme distrust (closer to 1) falls within a normal range.

*In summary*, the overall central tendency shows that most respondents are somewhat neutral or moderately distrusting, with few expressing extreme levels of trust. The distribution looks balanced, with not much extreme outliers that stand out as suspicious or extraordinary.

`resp_distributions` also provides two measures of *variability*: `ii_sd` (individual response variability) and `mahal` (deviation from overall response patterns).
-  The mean `ii_sd` is 1.13, meaning, on average, respondents’ trust levels fluctuate by about 1 point from their own personal average.
-  The third quartile of `ii_sd` is 1.45, meaning 75% of respondents have moderate variability in their responses, with some showing higher fluctuations. Those with variability exceeding 2.5 points are considered extreme within the group, and these respondents may need further scrutiny to ensure the reliability of their data.
-  Mahalanobis distance (`mahal`) does not have a straightforward interpretation like `ii_sd`. However, respondents with `mahal` scores slightly above 5 exhibit highly dissimilar response patterns compared to the overall average. These outliers could flag potential data quality concerns and requires further examination.

*In summary*, most respondents show moderate variability, but a few demonstrate significant deviations that may require further investigation to assess the consistency and reliability of their responses. Mahalanobis distance helps identify these respondents whose patterns deviate substantially from the group norm.
...
table(trust_distribution$non_diff)
...
The results show that about 5% of respondents show **straightlining response behavior** in the institutional trust scale, meaning that 5% of respondents selected an identical answer across the several items.
...
## NEP Scale Distributions

Now we will check central tendency and variability indicators for NEP scale;
...
# Create combined boxplot and density plot with a dashed line for each mean
ggplot(NEP_distribution_long, aes(x = Value, y = ..scaled..)) +
  geom_density(aes(y = ..scaled..), alpha = 0.5) +
  geom_boxplot(aes(y = -0.5), width = 0.5, outlier.size = 2, color = "black", fill = "lightgray") +
  geom_vline(data = mean_values, aes(xintercept = mean_value, color = "Mean"), linetype = "dashed") +
  scale_color_manual(values = c("Mean" = "black")) +
...
**Pay Attention to Reverse Coding**

Question: To what extend do you agree or disagree with the following statements? (1 = Fully Agree, ... , 5 = Fully Disagree)

1.  We are approaching the limit of the number of people the earth can support
*Pro-environmental (no need to reverse code)*
2.  Humans have the right to modify the natural environment to suit their needs
*Reverse code (negative)*
...
*Pro-environmental (no need to reverse code)*
6.  There are enough resources on the planet - we just have to learn how to use them
*Reverse code (negative)*
...
```{r}
# Environmental attitudes 
NEP_recoded_distribution <- resp_distributions(NEP_recoded)

# Summarize and print results over all respondents
NEP_recoded_table <- summary(NEP_recoded_distribution)
kable(NEP_recoded_table)

# Reshape the data for density and box plots
NEP_recoded_distribution_long <- pivot_longer(NEP_recoded_distribution, 
                                         cols = c("ii_mean", "ii_sd", "ii_median", "mahal"), 
                                         names_to = "Indicator", 
                                         values_to = "Value")

# Remove non-finite values
NEP_recoded_distribution_long <- NEP_recoded_distribution_long %>%
  filter(!is.na(Value) & is.finite(Value))

# Calculate mean for each Indicator
mean_values <- NEP_recoded_distribution_long %>%
  group_by(Indicator) %>%
  summarize(mean_value = mean(Value, na.rm = TRUE))

# Create combined boxplot and density plot with a dashed line for each mean
ggplot(NEP_recoded_distribution_long, aes(x = Value, y = ..scaled..)) +
  geom_density(aes(y = ..scaled..), alpha = 0.5) +
  geom_boxplot(aes(y = -0.5), width = 0.5, outlier.size = 2, color = "black", fill = "lightgray") +
  geom_vline(data = mean_values, aes(xintercept = mean_value, color = "Mean"), linetype = "dashed") +
  scale_color_manual(values = c("Mean" = "black")) +
...
```{r}
# Generating straightlining indicator for institutional trust scale 
NEP_recoded_distribution$non_diff <- NA 
NEP_recoded_distribution$non_diff[NEP_recoded_distribution$ii_sd == 0] <- 1
NEP_recoded_distribution$non_diff[NEP_recoded_distribution$ii_sd != 0] <- 0
table(NEP_recoded_distribution$non_diff)
# Get proportion of respondents who show straightlining response behavior
proportion_nondiff <- prop.table(table(NEP_recoded_distribution$non_diff))[2]
print(proportion_nondiff)

```

<!-- Where to include straightlining and which to use: NEP or NEP_recoded? -->
...
<!-- After some meetings I have an update regarding this paragraph: Let's not apply the `min_valid_responses`option but only shortly explain that this option exists and what it does. But that we generally recommend to exclude respondents with NAs, especially if the number of items in the multi-item scale is below 10. I can finalize this later on but let's just turn this paragraph for now into an information box with the heading *package-specific feature*.
