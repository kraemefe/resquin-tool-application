...
Environmental attitudes are measured with the established NEP (New Ecological Paradigm) scale @dunlap_et_al_2022 comprising 15 items on different aspects of environmental or climate attitudes. The multi-item scale uses a 5-point Likert response scale.

<!-- reference added for NEP - should be checked though -->
...
pacman::p_load(devtools, pak, dplyr)
```

As `resquin` is not currently stored in CRAN, we need to install the package from GitHub. For installation, we can use one of the following commands:
...
# Subset and inspect responses NEP 
