...
# Exemplary code to flag outliers displayed in the boxplots

# Calculate IQR and thresholds for each Indicator to determine the outliers we see in the boxplot
outlier_data_trust <- trust_distribution_long %>%
  group_by(Indicator) %>%
  summarize(
...
# Add a column to flag outliers in the dataset
trust_distribution_long <- trust_distribution_long %>%
  left_join(outlier_data_trust, by = "Indicator") %>%
  mutate(is_outlier = ifelse(Value < lower_threshold | Value > upper_threshold, 1, 0)) #Outliers take on the value 1 of the variable is_outlier

# Print variable
table(trust_distribution_long$is_outlier, useNA="ifany")
...
-   Ideally, we recommend to conduct further analyses on respondents
    ...
    *response times*. If respondents show extremely short response times
    ...
    Further measures to approach the question whether respondents' answers
    reflect low engagement with a question and therefore, poor quality
    responses are *respondent motivation* or *respondent cognitive ability*.
    These are both factors discussed to influence the occurrence and
    magnitude of *satisficing*, an umbrella term for different response
    strategies that can be used as shortcuts to reduce cognitive burden
    and can therefore, reflect careless responding (@krosnick1991response).
